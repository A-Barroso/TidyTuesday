setwd("C:/Users/Nutzer/Desktop/MSc/R/Projects/TidyTuesday")

library(tidyverse)
library(extrafont)
library(cowplot)

font_import(GABRWFFR)

friends_emotions <- read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-09-08/friends_emotions.csv')

friends.clean <- friends_emotions %>%
  select(-episode, -scene, -utterance) %>%
  mutate(
    season = case_when(
      season == "1" ~ "Season 1",
      season == "2" ~ "Season 2",
      season == "3" ~ "Season 3",
      season == "4" ~ "Season 4")
  ) %>%
  group_by(season) %>%
  count(emotion)


img <- "https://pbs.twimg.com/media/EhfwFiEWAAIVEsL?format=png&name=4096x4096"

friends.clean$emotion <- as.factor(friends.clean$emotion)
friends.clean$emotion <- factor(friends.clean$emotion, levels = c("Joyful",
                                                                  "Mad",
                                                                  "Peaceful",
                                                                  "Sad",
                                                                  "Powerful",
                                                                  "Scared",
                                                                  "Neutral"))

p <- ggplot(friends.clean, aes(emotion, n, fill = emotion)) +
  geom_col() +
  coord_polar(theta = "y", clip = "off") +
  theme_minimal() +
  facet_wrap(~ season, nrow = 1) +
  labs(title = "Friends: An Emotional Rollercoaster",
       subtitle = "Aggregated emotional values per season",
       caption = "@BA_Barroso | Source: friends R Package | #TidyTuesday Week 37") +
  scale_fill_manual(values = c("#F74035", "#9787CD", "#3F9DD4", "#008F48", "#F6D400", "#DF4013", "#941205")) +
  guides(fill = guide_legend(nrow = 1, override.aes = list(size = 2))) +
  theme(text = element_text(family = "Open Sans"),
        plot.margin = margin(20,20,20,20),
        legend.position = "bottom",
        legend.title = element_blank(),
        legend.text = element_text(colour = "grey97"),
        strip.text = element_text(colour = 'grey97'),
        panel.grid.major = element_line(size = 0.3, colour = "grey10"),
        panel.grid.minor = element_line(size = 0.3, colour = "grey10"),
        axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        plot.background = element_rect(fill = "#000000"),
        plot.title = element_text(size = 28, face ="bold", colour = "grey97", hjust = 0.8, margin = margin(20, 0, 10, 0)),
        plot.subtitle = element_text(size = 12, colour = "grey97", hjust = 0.5, margin = margin(0, 0, 30, 0)),
        plot.caption = element_text(size = 6, colour = "grey97", hjust = 0.5)) +
  geom_text(label = "IMDb Rating: X",
            family = "Open Sans",
            size = 5,
            x = -8,
            y = 1,
            colour = "white")

### WHAT IF: do label = data, where data <- individual character values for rating per season group

friends_info <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-09-08/friends_info.csv')

# ratings <- friends.clean$season

f_info.clean <- friends_info %>%
  select(season, imdb_rating) %>%
  mutate(
    season = case_when(
      season == "1" ~ "Season 1",
      season == "2" ~ "Season 2",
      season == "3" ~ "Season 3",
      season == "4" ~ "Season 4")
  ) %>%
  group_by(season)

median(f_info.clean$imdb_rating ~ season)

## mean or median? also it wont let me do the ~ season part so....?

ggdraw() +
  draw_plot(p) +
  draw_image(img, scale=0.30, y=0.28, x=-0.219)

ggsave("TidyTuesdayW37.tiff", plot = last_plot(), dpi = 400, height = 180, width = 280, units = "mm")







family = "Open Sans",
size = 5,
x = -8,
y = 1,
colour = "white")



